...
};b.prototype={init:function(){var c=this;c.$buttonDelete.tooltip();c.$buttonDelete.on("click",function(){if(c.blockButtons){return c}ls.progressStart();
c.showLoader(true);c.blockButtons=true;ls.ajax(c.options.url.remove,{options:c.options},function(d){ls.progressDone();c.hideLoader(true);c.blockButtons=false;
c.$buttonDelete.hide();c.options.brandingBackground="";c.options.brandingBackgroundType="";c.options.brandingHeaderStep=0;c.options.brandingHas=false;c.options.brandingOpacity=100;
c.options.brandingUseBackgroundColor=false;c.options.brandingBackgroundColor="#FFFFFF";c.options.brandingFontColor="#000000";c.options.brandingHeaderColor="#000000";
c.formCss();c.$headerSlider.val(c.options.brandingHeaderStep).change();c.$image.attr("src",a(".js-alto-uploader").data("empty"));c.$backgroundType.show();
c.$backgroundType.find("[value='0']").attr("selected","selected");c.$buttonRemove.fadeOut(0);c.$buttonUpload.fadeIn(0);c.$backgroundType.parents(".uploader-blog-type").fadeIn(0);
c.$buttonColorOnly.fadeIn(0);c.$buttonBoth.fadeOut(0);c.$file.fadeIn(0);c.$image.css("opacity",1);c.$slider.val(100).change();c.$sliderLabel.text("100%");
c.$backgroundPicker.val(c.options.brandingBackgroundColor).css({"background-color":"#FFFFFF"}).css({color:jQuery.Color("#FFFFFF").contrastColor()});c.$fontColorPicker.val(c.options.brandingFontColor).css({"background-color":"#000000"}).css({color:jQuery.Color("#000000").contrastColor()});
c.$headerColorPicker.val(c.options.brandingHeaderColor).css({color:jQuery.Color("#000000").contrastColor()}).css({"background-color":"#000000"});if(!d){ls.msg.error(null,"System error #1001")
}else{ls.msg.notice(null,ls.lang.get(c.options.langBlogBrandingRemoved))}})});jQuery.Color.fn.contrastColor=function(){var f=this._rgba[0],e=this._rgba[1],d=this._rgba[2];
...
}c.$backgroundType.parents(".uploader-blog-type").fadeIn(0);c.$buttonColorOnly.fadeIn(0);c.$buttonBoth.fadeOut(0);c.$file.css("top",0);c.$image.css("opacity",1);
c.$file.css("display","block");c.formCss()});c.$buttonSave.on("click",function(){c.saveBranding()});ls.hook.add("uploader_upload_image_after",function(e,d){if(e.target=="blog-branding"||e.target=="topic-branding"||e.target=="user-branding"){var f=a("body");
...
}return c},showLoader:function(c){},hideLoader:function(c){},formCss:function(){var g=this;if(g.options.brandingHas==false){g.$styles.remove();g.$styles=a('<div id="plugin-branding-blog-styles"><style></style></div>');
...
f+=" #footer { position: relative; top: "+g.options.brandingHeaderStep+"px; }"}else{f+=" "+g.options.contentSelector+"{ position: inherit; top: 0px; }";
f+=" #footer { position: inherit; top: 0px; }"}g.$styles.remove();g.$styles=a('<div id="plugin-branding-blog-styles"><style>'+f+"</style></div>");g.$styles.appendTo("body");
return g},getBackgroundColor:function(){var c=this;return"rgba("+c.hexToR(c.options.brandingBackgroundColor)+", "+c.hexToG(c.options.brandingBackgroundColor)+", "+c.hexToB(c.options.brandingBackgroundColor)+", "+(c.options.brandingOpacity/100).toString()+")"
